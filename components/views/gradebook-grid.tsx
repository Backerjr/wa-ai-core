...
export function GradebookGrid({
  ...
  const [records, setRecords] = React.useState(() => rows.map((row) => ({ ...row, final: calculateFinalGrade(row) })));

  const updateScore = (id: string, field: keyof Pick<GradebookRow, "assignment" | "project" | "assessment">, value: number) => {
    setRecords((prev) =>
      prev.map((row) =>
        row.id === id
          ? {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <h2 className="text-2xl font-semibold text-[var(--color-fg)]">{classNameLabel}</h2>
            <Select
              value={classId}
          ...
          <p className="text-xs text-slate-500">Weights — Assignments 40%, Projects 30%, Assessments 30%</p>
        </div>
      ...
      <div className="overflow-x-auto rounded-3xl border border-white/40 bg-white/80 shadow-card">
        <table className="min-w-full divide-y divide-slate-100">
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((row) => (
              <tr key={row.id}>
                <td className="px-6 py-4 text-sm font-semibold text-[var(--color-fg)]">{row.name}</td>
                {(["assignment", "project", "assessment"] as const).map((key) => (
                  <td key={key} className="px-6 py-4">
                    <input
                      ...
                      onChange={(event) => updateScore(row.id, key, Number(event.target.value))}
                ...
                <td className="px-6 py-4 text-lg font-semibold text-[var(--color-fg)]">{row.final}%</td>
